...
class BudgetViewModel @Inject constructor(
    ...
    companion object {
        ...
        private const val WARNING_PERCENT = 80.0 // percent
    ...
    fun loadBudget(userId: String) {
        ...
        budgetRepository.getCurrentMonthBudgetFlow(userId)
            ...
            .catch { e ->
                _uiState.update { it.copy(isLoading = false, error = e.message ?: "Failed to load budget") }
            }
                ...
                onSuccess = { budget ->
                    _uiState.update {
                        it.copy(
                            isLoading = false,
                            hasBudget = true,
                            successMessage = "Budget set to SGD ${"%.2f".format(amount)}"
                        )
                    }
                },
                onFailure = { err ->
                    _uiState.update { it.copy(isLoading = false, error = err.message ?: "Failed to set budget") }
                }
            )
        }
    }
    ...
    fun addSpending(userId: String, amount: Double) {
        ...
        viewModelScope.launch {
            ...
            result.fold(
                ...
                onFailure = { err ->
                    _uiState.update { it.copy(error = err.message ?: "Failed to add spending") }
                }
            )
        }
    }
    ...
    fun removeSpending(userId: String, amount: Double) {
        ...
        viewModelScope.launch {
            ...
            result.fold(
                onSuccess = {
                    _uiState.update { it.copy(successMessage = "Refund of SGD ${"%.2f".format(amount)} applied") }
                },
                onFailure = { err ->
                    _uiState.update { it.copy(error = err.message ?: "Failed to remove spending") }
                }
            )
        }
    }
    ...
    fun getBudgetStatus(): BudgetStatus {
        ...
        return when {
            percentage >= CRITICAL_PERCENT -> BudgetStatus.EXCEEDED
            percentage >= WARNING_PERCENT -> BudgetStatus.WARNING
...
data class BudgetUiState(
    ...
    val usagePercentage: Double = 0.0, // 0..100
